...
이 프로젝트의 과제 목표는 Docker와 Docker Compose를 사용하여 최소 3개 컨테이너로 구성된 3-tier 웹서비스를 직접 설계하고, 각 계층의 역할과 연결 방식을 문서화하는 것입니다.

Study Note Manager는 학습 노트를 관리하는 웹서비스를 주제로 하며, 다음과 같이 과제 요구사항에 대응합니다.

| 과제 요구사항 | 이 프로젝트의 구현 |
| --- | --- |
| Presentation Tier | `frontend` 컨테이너가 HTML/CSS/JavaScript 화면과 Nginx reverse proxy 제공 |
| Application Tier | `backend` 컨테이너가 Express REST API와 CRUD/검색/필터 비즈니스 로직 제공 |
| Data Tier | `mysql` 컨테이너가 MySQL 8.0 DB와 초기 테이블, volume 기반 데이터 저장 제공 |
...
과제 제출 시 실제 이미지 또는 시연 영상을 저장소에 포함하지 않고 과제 제출 시스템에 별도 첨부하는 것을 권장합니다. 아래 체크리스트에 맞춰 캡처 또는 영상을 준비합니다.
...
실제 캡처 이미지 또는 시연 영상은 저장소 경로가 아니라 과제 제출 시스템에 별도로 첨부합니다.
...
- 실제 실행 비밀번호는 `.env.example`이 아니라 로컬 전용 `.env`에만 작성합니다.
