...
- 프로젝트에 적용한 내용: 실제 서비스명을 `frontend`, `backend`, `mysql`로 정하고 README에 계층별 역할을 정리했다.
- 직접 확인한 내용: 과제 요구사항과 `docker-compose.yml` 구조를 직접 비교하여 수정했다.
...
- 프로젝트에 적용한 내용: 현재 폴더 구조에 맞게 Dockerfile, SQL 초기화 파일, 문서 위치를 정리했다.
- 직접 확인한 내용: 불필요한 폴더는 만들지 않고 필요한 구조만 사용했다.
...
- 프로젝트에 적용한 내용: `/api/health`, `/api/notes` 중심으로 backend를 구성했다.
- 직접 확인한 내용: 실제 endpoint는 `curl`과 코드 확인으로 검증했다.
...
- 프로젝트에 적용한 내용: `db/init/01_init.sql`의 `notes` 테이블 구성에 적용했다.
- 직접 확인한 내용: backend에서 사용하는 필드명과 맞도록 직접 수정했다.
...
- 프로젝트에 적용한 내용: `frontend/index.html`, `frontend/css/style.css`, `frontend/js/app.js`에 화면과 기능을
  연결했다.
- 직접 확인한 내용: 화면에서 직접 입력, 수정, 삭제 흐름을 확인했다.
...
- 프로젝트에 적용한 내용: `docker-compose.yml`에 세 서비스와 `mysql-data` volume을 구성했다.
- 직접 확인한 내용: `docker compose config`와 실행 테스트로 확인했다.
...
- 프로젝트에 적용한 내용: `frontend/nginx.conf`에 `proxy_pass http://backend:5001/api/;` 설정을 작성했다.
- 직접 확인한 내용: 브라우저와 `curl`로 frontend/backend 연결을 확인했다.
...
- 프로젝트에 적용한 내용: Compose healthcheck와 backend DB 연결 재시도 로직을 적용했다.
- 직접 확인한 내용: `docker compose ps`와 `/api/health`로 확인했다.
...
- 프로젝트에 적용한 내용: README에 프로젝트 개요, 구조, 흐름, 포트, 실행 방법을 정리했다.
- 직접 확인한 내용: 실제 설정과 다르지 않도록 `docker-compose.yml`과 코드를 계속 대조했다.
...
- 프로젝트에 적용한 내용: `.env.example` placeholder 정리, `.env` 비추적 확인, README 보안 안내 보완에 적용했다.
- 직접 확인한 내용: `grep`과 Git 추적 상태로 확인했다.
...
- 프로젝트에 적용한 내용: 검토 결과를 바탕으로 backend/mysql 로컬 바인딩, CORS `*` 제거, Dockerfile 개선 사항을 직접 적용했다.
- 직접 확인한 내용: build, up, curl 테스트로 직접 확인했다.
...
- 프로젝트에 적용한 내용: README와 AI_PROMPTS를 과제 기준에 맞게 정리했다.
- 직접 확인한 내용: 실제 실행 테스트 결과와 비교했다.
...
- 프로젝트에 적용한 내용: 최종 검증 명령과 제출 전 체크리스트를 문서에 정리했다.
- 직접 확인한 내용: 직접 명령을 실행한 뒤 결과를 확인했다.
...
- 프로젝트에 적용한 내용: 검토 결과를 바탕으로 Compose 비밀번호 fallback 제거, backend `DB_PASSWORD` 필수 검증, README 안내
  추가를 직접 적용했다.
- 직접 확인한 내용: `docker compose config`, `grep`, backend check로 확인했다.
