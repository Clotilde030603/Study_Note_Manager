...
이 문서는 Study Note Manager를 만들면서 ChatGPT를 참고한 내용을 과제 제출용으로 정리한 기록이다.
주제 선정, 3-tier 구조 선택, 코드 수정, 실행 테스트는 직접 진행했고,
ChatGPT는 구조를 확인하거나 막힌 부분의 방향을 잡을 때 보조로 사용했다.

ChatGPT 답변은 바로 붙여 넣기보다는 `docker-compose.yml`, backend/frontend 코드,
실행 결과와 맞는지 다시 확인한 뒤 필요한 부분만 프로젝트에 맞게 적용했다.
실제 비밀번호나 토큰 같은 민감정보는 기록하지 않고 placeholder만 사용했다.
...
ChatGPT는 설계나 문서 방향을 확인하는 용도로 사용했다.
특히 Docker Compose 구조, API 경로, 환경변수, 보안 설정은 답변만 보고 넘기지 않고
실제 파일과 실행 결과를 기준으로 다시 맞춰 보았다.
